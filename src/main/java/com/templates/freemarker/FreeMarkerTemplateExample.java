...
public class FreeMarkerTemplateExample {
    ...
    public static final FreeMarkerTemplateEngine engine = FreeMarkerTemplateEngine.create(vertx);
    ...
    public static void main(String[] args) {
        vertx.deployVerticle(new EmailSender())
                .compose(result -> {
                    ...
                    // Render the template using FreeMarker with the JsonObject data
                    engine.render(data, "src/main/resources/template.ftl", res -> {
                        if (res.succeeded()) {
